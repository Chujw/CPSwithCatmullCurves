...
struct DeadTraces{
	makePDF deadtraces;
	int feedfrom; // beacon id of the particle who kills it
	bool needfeed; // does it still need to be feed more points
	int sizewhenkill; // path size of the particle who kills it
};
...
private:
	...
	vector<DeadTraces> deadpar;
	//vector<makePDF> SecondGroupPars;
	bool* Foreground_map;
	int head_par;
	int rear_par;
	...
	long int beacon_num;
	bool Is_Foreground;// is this par group works for foreground area?
...
public:
	...
	void Drawvectorcurves();
	void KeepDeadtraces(makePDF* deadpar, float x, float y, Particle* feedpar);
	void KeepDeadtracesAndExpand(makePDF* deadpartrace, float x, float y);
	void FeedAllDeadPars();
	//Particle FindPar(float x, float y);
	Particle* FindPar(int beaconID);
	void StoreCatmullPath();
};