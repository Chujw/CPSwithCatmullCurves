...
//---------------------------------------------
//
// Display all particle traces in bitmap
//
//---------------------------------------------
...
//---------------------------------------------
//
// Display all particle traces in vectore form
//
//---------------------------------------------
void ParticleGroups::Drawvectorcurves()
{
	for(int i=0; i<numpt; i++)
	{
		// draw vector form catmull curves
		particle[i].makepdf.drawpath(Is_Foreground);
	}

	for(int i=0; i<deadpar.size(); i++)
	{
		//draw traces for all dead pars
		deadpar[i].deadtraces.drawpath(Is_Foreground);
	}
}

void ParticleGroups::KeepDeadtraces(makePDF* deadpartrace, float x, float y, Particle* feedpar)
{
	//make a dead par which have all the traces points, and remember who killed it
	DeadTraces thisdeadpar;
	thisdeadpar.deadtraces = *deadpartrace;
	thisdeadpar.deadtraces.growpath(x,y,true,true);
	thisdeadpar.feedfrom = feedpar->beacon_id;
	thisdeadpar.needfeed = true;
	thisdeadpar.sizewhenkill = feedpar->makepdf.GetPathSize();
	deadpar.push_back(thisdeadpar);

}

void ParticleGroups::KeepDeadtracesAndExpand(makePDF* deadpartrace, float x, float y)
{
	//make a dead par which have all the traces points, and expand it based on its own path
	DeadTraces thisdeadpar;
	thisdeadpar.deadtraces = *deadpartrace;
	thisdeadpar.deadtraces.growpath(x,y,true,true);
	//thisdeadpar.feedfrom = -1;
	thisdeadpar.needfeed = false;
	//thisdeadpar.sizewhenkill = 0;
	...
	if(thisdeadpar.deadtraces.GetPathSize()>=2)
	{
		for(int iter = 0; iter<4; iter++)
		{
			...
			float k = (p2->y-p1->y)/(p2->x-p1->x); // slope
			float p3x, p3y;
			p3x = p2->x + (p2->x-p1->x);
			p3y = k*(p3x-p2->x)+p2->y;
			thisdeadpar.deadtraces.growpath(p3x,p3y,true,true);
		}
	}
	deadpar.push_back(thisdeadpar);
}

void ParticleGroups::FeedAllDeadPars()
{
	for(int i = 0; i<deadpar.size(); i++)
	{
		if(deadpar[i].needfeed)
		{
			// find the par who killed it
			Particle* thisfeedpar = FindPar(deadpar[i].feedfrom);
			if(thisfeedpar) // if this par is still alive
			{
				//if this par has more points since kill
				int diffsize = thisfeedpar->makepdf.GetPathSize()-deadpar[i].sizewhenkill;
				if(diffsize < DIFF_SIZE)
				{
					if(diffsize > 0)
					{
						ofPoint* thisfeedpoint = thisfeedpar->makepdf.GetPoint(thisfeedpar->makepdf.GetPathSize()-1);
						deadpar[i].deadtraces.growpath(thisfeedpoint->x,thisfeedpoint->y,true,true);
						deadpar[i].needfeed = true;
					}
				}
				else
					deadpar[i].needfeed = false;
			}
			else
				deadpar[i].needfeed = false;
		}
...
void ParticleGroups::Simulate(SpacingMap* m_spacing, ofImage* m_canvas)
{
	if(!canstop)
	{
		...
		{
			int thisparID = indexPar->id;	// mark thispar's ID as the current one
			indexPar = indexPar->next;		// let the index pointer point to the next one
			TiltUpdate(&particle[thisparID]);	
			particle[thisparID].Update();	// update position
...
void ParticleGroups::StoreCatmullPath()
{
	for(int i=0;i<numpt;i++)
	{
...
{
	//keep the traces for making catmull curves
...
Particle* ParticleGroups::FindPar(int beaconID)
{
	for(int i=0; i<particle.size(); i++)
	{
		if(particle[i].beacon_id == beaconID)
			return &particle[i];
	}
