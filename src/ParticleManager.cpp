...
void ParticleManager::Setup(ofImage* edgeImage, ofImage* srcImage)
{
	...
	FillObjects = false;
	Saved1stGroup = false;
	parGroup.resize(GROUP_NUM);
	edgemap.resize(GROUP_NUM);
...
void ParticleManager::drawcatmull()
{
	parGroup[0].Drawvectorcurves();

	if(parGroup[0].GetStopSignal())
	{
		parGroup[1].Drawvectorcurves();
	}
}
...
bool ParticleManager::GetSaved1stGroup()
{
	return Saved1stGroup;
}

bool ParticleManager::Get1stGroupStopSign()
{
	return parGroup[0].GetStopSignal();
}

void ParticleManager::SetSaved1stGroup()
{
	Saved1stGroup = true;
}